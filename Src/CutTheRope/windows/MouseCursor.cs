...
namespace GameManager.windows
{
    internal class MouseCursor
    {
        ...
        {
            _cursorWindows = default;//NativeMethods.LoadCustomCursor("content/cursor_windows.cur");
            _cursorActiveWindows = default;//NativeMethods.LoadCustomCursor("content/cursor_active_windows.cur");
        }

        public void Draw()
        {
            if (_enabled && !Global.XnaGame.IsMouseVisible && _mouseStateOriginal.X >= 0 
                && _mouseStateOriginal.Y >= 0)
            {
                Texture2D texture2D = ((_mouseStateTranformed.LeftButton 
                    == Microsoft.Xna.Framework.Input.ButtonState.Pressed) ? _cursorActive : _cursor);
